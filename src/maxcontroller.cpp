...
void connectWiFi_AP() {
  ...
  delay(15);
  printf("INFO: Creating Access point...\n");
  printf("INFO: AP ipAddress: %d.%d.%d.%d\n", apAddress[0], apAddress[1], apAddress[2], apAddress[3]);
  printf("INFO: AP ssid: %s, AP password: %s\n", ssid, pass);
}
...
void startUdpServer() {
  ...
  if(res == 1) 
    printf("INFO: Opened UDP socket on port %d\n", PORT_IN);
  else 
    printf("***ERROR: Error opening UDP socket\n");
  delay(15);
...
boolean connectWiFi() {
  ...
  if(WiFi.SSID() == settings.network && WiFi.psk() == settings.password) {
    printf("INFO: connect: Loading WiFi settings from ROM\n");
    WiFi.begin();
  }
  else {
      printf("INFO: Connecting with new settings\n");
  ...
  while (WiFi.status() != WL_CONNECTED) {
    delay(250);
    Serial.print("*.");
   if (i > 80){              // Wait 8s for connecting to WiFI
    state = false;
      break;
    }
    i++;
  ...
  if(!state) {
    printf("***ERROR: Cant connect to WiFi SSID: %s\n", settings.network);
    WiFi.disconnect();
  }
  else {
    IPAddress ip = WiFi.localIP();
    printf("INFO: Connected to WiFi! SSID: %s, ip: %d.%d.%d.%d\n", settings.network, ip[0], ip[1], ip[2], ip[3]);
  }
  return state;
}
